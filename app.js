...
function clearGroup(group){ while(group.children.length){ const o=group.children.pop(); o.traverse?.(n=>{n.geometry?.dispose?.(); if(n.material){(Array.isArray(n.material)?n.material:[n.material]).forEach(m=>m.dispose?.());}}); } }

function makeWall(a,b,height=3.4){
  const dx=b[0]-a[0], dz=b[1]-a[1], len=Math.hypot(dx,dz);
  const geo=new THREE.BoxGeometry(len,height,.45);
  const mat=new THREE.MeshStandardMaterial({color:0x6f8799,roughness:.78,metalness:.05,transparent:true,opacity:.88});
  const m=new THREE.Mesh(geo,mat); m.position.set((a[0]+b[0])/2,height/2,(a[1]+b[1])/2); m.rotation.y=-Math.atan2(dz,dx); return m;
}

function buildWalls(){
  const group=new THREE.Group(); group.name='walls';
  // 依原圖主要建築輪廓做的 V1 立體外框；精細牆線後續可逐段校正
  const p=[[-45,-31],[36,-31],[45,-18],[45,12],[42,31],[25,35],[-28,35],[-46,27],[-46,-3],[-45,-31]];
  for(let i=0;i<p.length-1;i++) group.add(makeWall(p[i],p[i+1]));
  // 幾個主要核心/分區牆，增加立體辨識度
  [[[-33,-22],[-20,-22]],[[-20,-22],[-20,-10]],[[-33,-10],[-20,-10]],
   [[18,-11],[29,-11]],[[18,-11],[18,2]],[[29,-11],[29,2]],
   [[-33,15],[-20,15]],[[-20,15],[-20,28]],[[-33,28],[-20,28]]].forEach(seg=>group.add(makeWall(seg[0],seg[1],2.8)));
  group.visible=state.showWalls; floorRoot.add(group);
}
...
function buildFloor(){
  ...
  floorPlane.rotation.x=-Math.PI/2; floorPlane.position.y=0; floorPlane.userData.isFloor=true;
