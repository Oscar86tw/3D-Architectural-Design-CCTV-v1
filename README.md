...
## Google API 更新
V1.45 的 `2.GOOGLE API/Code.gs` 支援雲端資料夾管理，並新增專案檔鎖定 API：
- `listFolders`
- `saveFolder`
- `deleteFolder`
- `setProjectLock`
...
- 原因：V1.45 產生壓縮檔時 `Code.gs` 被重複拼接第二份完整程式，並形成 `}onst CCTV_SPREADSHEET_ID`。
...
### V1.45
- 修正 V1.45～V1.45 重構通訊時遺失的「本地儲存」「匯出」「匯入」「雲端儲存」事件處理。
...
- V1.45 查找案件只讀 A:I 索引欄；只有 Drive 讀取失敗時才讀該案件自己的 J 欄備援 JSON。
